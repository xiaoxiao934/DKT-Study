...
parser.add_argument('-train_data_path', type=str, default='data/builder_train.csv', help='Path to the training dataset')
parser.add_argument('-test_data_path', type=str, default='data/builder_test.csv', help='Path to the testing dataset')
...
def run_epoch(m, optimizer, students, batch_size, num_steps, num_skills, training=True, epoch=1):
    ...
    while (index + batch_size < len(students)):
        ...
        x = torch.unsqueeze(x, 2)  # 用于在 x 张量的第三个维度（索引为2）上添加一个维度
        ...
        if training:
            ...
            loss = criterion(logits, target_correctness)  # 计算模型的预测结果 logits 和真实标签 target_correctness 之间的损失值
        ...
        # 测试
            ...
            # 在测试阶段，我们只关心模型的输出结果，而不需要计算梯度或进行参数更新。因此，通过禁用梯度计算，可以提高测试阶段的效率并减少内存消耗
...
def main():
    ...
    num_steps = train_max_num_problems  # 表示模型在每个学生的序列中将处理的时间步数
    num_skills = train_max_skill_num
    num_layers = 1  # 模型中的LSTM层数
    ...
    for i in range(args.epochs):
        ...
        if ((i + 1) % args.evaluation_interval == 0):
            rmse, auc, r2 = run_epoch(model, optimizer, test_students, batch_size, num_steps, num_skills,
                                      training=False)
            print('Testing')
            print(rmse, auc, r2)
